...
{#if $aside}
  <aside
    class="fixed top-0 z-20 h-full w-72 border-r border-neutral-700 bg-app-bg"
    use:clickOutside
    on:outclick={aside.toggle}
    in:fly={{ x: -200, duration: 350 }}
  >
    <div
      class="flex h-full w-full flex-col p-4"
      transition:fly={{ y: 15, duration: 350, delay: 300 }}
    >
      ...
      {#each $routes as route}
        <a
          class="route w-full border-b border-neutral-700 p-2 font-title text-xl"
          href={route.src}
